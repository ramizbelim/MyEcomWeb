...
def index(request):
    ...
    dictionary = {'allProds': allProds}
    return render(request, 'shop/index.html',dictionary)

def searchMatch(query, item):
    '''return true only if query mathces the items'''
...
def search(request):
    ...
    if len(allProds) == 0 or len(query)<4:
        dictionary = {'msg':"Please make sure to enter relevant search query"}
...
def handlerequest(request):
    ...
    verify = Checksum.veryfy_checksum(response_dict, MERCHANT_KEY,checksum)
    if verify:
